...
import { Loader2, RefreshCw, Star, Plus, Bot, Eye, FileText } from "lucide-react";
            ...
            async ([entry]) => {
                if (entry.isIntersecting) {
                    ...
                    if (!sessionStorage.getItem(viewedKey)) {
                        ...
                        setViewCount(v => v + 1);
                        const itemRef = doc(db, 'feedItems', item.id);
                        try {
...
function FeedPageComponent() {
  ...
  const addPostForm = useForm<AddPostFormValues>({ resolver: zodResolver(addPostFormSchema), defaultValues: { headline: "", content: "", url: "" } });
  ...
  const onAddPostSubmit: SubmitHandler<AddPostFormValues> = async (values) => {
    ...
    try {
        await addDoc(collection(db, "feedItems"), {
            ...
            url: values.url || "#", likes: 0, comments: 0, views: 0, createdAt: serverTimestamp(), userId: user.uid,
        });
                ...
                <div className="mx-auto max-w-3xl">
                    <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="item-1">
                            ...
                            <AccordionContent><div className="space-y-4 pt-4">{topicHistory.map(topic => (
          ...
          <Dialog open={isAddPostOpen} onOpenChange={setIsAddPostOpen}>
            ...
            <DialogContent><DialogHeader><DialogTitle>Create a New Post</DialogTitle><DialogDescription>Share your latest thoughts and insights with the community.</DialogDescription></DialogHeader>
                 <Form {...addPostForm}><form onSubmit={addPostForm.handleSubmit(onAddPostSubmit)} className="space-y-4">
                        ...
                        <FormField control={addPostForm.control} name="content" render={({ field }) => ( <FormItem><FormLabel>Content</FormLabel><FormControl><Textarea placeholder="Share your detailed thoughts here..." className="min-h-[160px]" {...field} /></FormControl><FormDescription>Markdown is supported for formatting.</FormDescription><FormMessage /></FormItem> )}/>
